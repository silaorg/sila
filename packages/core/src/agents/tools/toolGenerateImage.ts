...
export function getToolGenerateImage(
  ...
  return {
    ...
    handler: async (args: Record<string, any>): Promise<GenerateImageResult> => {
      ...
      try {
        ...
        if (inputFiles && Array.isArray(inputFiles) && inputFiles.length > 0) {
          ...
          for (const filePath of inputFiles) {
            ...
            try {
              ...
              inputVertex = resolver.pathToVertex(filePath, relativeRootVertex);
            } catch (error) {
              const errorMessage = error instanceof Error ? error.message : String(error);
              return {
                status: "failed",
                message: `Input file not found: ${filePath} (${errorMessage})`,
              };
            }
          ...
          if (images.length === 0) {
            return {
              status: "failed",
              message: "No valid image files found in the provided file paths",
            };
          }
        ...
        if (!targetTree) {
          return {
            status: "failed",
            message: "Could not determine target app tree for saving generated images",
          };
        }
        ...
        for (let i = 0; i < result.urls.length; i++) {
          ...
          if (!response.ok) {
            console.error(`Failed to download generated image ${i + 1}: ${response.status}`);
            continue;
          }
          ...
          if (typeof Buffer !== "undefined") {
            ...
            const binaryString = Array.from(imageBytes, (byte: number) => String.fromCharCode(byte)).join("");
          ...
          if (outputPath && i === 0 && result.urls.length === 1) {
            ...
            const { parent, name } = ensureFileParent(space, appTree, finalPath);
            parentFolder = parent;
            fileName = name;
          } else {
