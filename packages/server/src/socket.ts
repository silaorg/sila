...
import { StateVector, type VertexOperation } from "reptree";
import {
...
async function getTreeOpsDiffAndVector(
  ...
  if (treeId === space.getId()) {
    ...
    const stateVector = StateVector.fromOperations(space.tree.getAllOps() as VertexOperation[]).getState();
  ...
  const stateVector = StateVector.fromOperations(appTree.tree.getAllOps() as VertexOperation[]).getState();
  return { ops, stateVector };
}
