...
import { loadWorkspaceAgentInstructions } from "../src/agent-instructions.js";

test("loadWorkspaceAgentInstructions overrides default base instructions with custom files", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "heswe-instructions-"));
  const workspacePath = path.join(tempRoot, "workspace");
  const instructionsDir = path.join(workspacePath, "agents", "default", "instructions");
  ...
  const instructions = await loadWorkspaceAgentInstructions(workspacePath, "slack");
...
test("loadWorkspaceAgentInstructions falls back to default instructions", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "heswe-instructions-"));
  const workspacePath = path.join(tempRoot, "workspace");
...
test("loadWorkspaceAgentInstructions ignores binary files in custom instructions directory", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "heswe-instructions-"));
  const workspacePath = path.join(tempRoot, "workspace");
  const instructionsDir = path.join(workspacePath, "agents", "default", "instructions");
