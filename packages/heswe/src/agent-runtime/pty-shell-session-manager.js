...
export class PTYShellSessionManager {
  ...
  async execute(sessionId, command) {
    ...
    const startToken = `__HESWE_CMD_START_${commandId}__`;
    const endToken = `__HESWE_CMD_END_${commandId}__`;
    ...
    return new Promise((resolve) => {
      ...
      const wrapped =
        `__HESWE_ESC='${escapedCommand}'; ` +
        `__HESWE_CMD="$(printf '%b' "$__HESWE_ESC")"; ` +
        `printf '\\n${startToken}\\n'; ` +
        `eval "$__HESWE_CMD"; ` +
        `__HESWE_EXIT=$?; ` +
        `__HESWE_CWD="$(pwd -P 2>/dev/null || pwd)"; ` +
        `printf '\\n${endToken}:%s:%s\\n' "$__HESWE_EXIT" "$__HESWE_CWD"; ` +
        `unset __HESWE_ESC __HESWE_CMD __HESWE_EXIT __HESWE_CWD`;
